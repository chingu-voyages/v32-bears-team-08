const service = require("./rejected.service");
...
async function find(req, res, next) {
    const response = await service.find(req.params.rejected_id);
    if (response[0]) {
        return res.json({ 
            data: response[0],
        });
    ...
    next({
        status: 404,
        message: `rejected ${req.params.rejected_id} not found`,
    })
}
...
async function remove(req, res, next) {
    const response = await service.remove(req.params.rejected_id);
    return res.json({
        data: response[0],
    })
}
