const service = require("./accepted.service");
...
async function find(req, res, next) {
    const response = await service.find(req.params.accepted_id);
    if (response[0]) {
        return res.json({ 
            data: response[0],
        });
    ...
    next({
        status: 404,
        message: `accepted ${req.params.connection_id} not found`,
    })
}
...
async function remove(req, res, next) {
    const response = await service.remove(req.params.accepted_id);
    return res.json({
        data: response[0],
    })
}
...
function hasAnchor(req, res, next) {
    ...
    next({
        status: 400,
        message: "request body data must have anchor property",
    })
}
...
function hasTarget(req, res, next) {
    ...
    next({
        status: 400,
        message: "request body data must have target property",
    })
}
