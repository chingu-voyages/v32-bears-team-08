...
async function find(req, res, next) {
    const response = await service.find(req.params.opportunity_id);
    if (response[0]) {
        return res.json({ 
            data: response[0],
        });
    ...
    next({
        status: 404,
        message: `opportunity ${req.params.opportunity_id} not found`,
    })
}
...
async function remove(req, res, next) {
    const response = await service.remove(req.params.opportunity_id);
    return res.json({
        data: response[0],
    })
}
...
function hasUserSkill(req, res, next) {
    ...
    next({
        status: 400,
        message: "request body data must have user-skill property",
    })
}
